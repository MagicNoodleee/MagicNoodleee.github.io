...
**非英文写作的论文** — 给它加一行 `lang: "zh"`，英文界面下标题后会自动跟一个淡色的
`(in Chinese)`，中文界面下不显示。支持 `zh` / `ja` / `de` / `fr`，要加别的语种改
`common.js` 里的 `LANG_NOTE`。这个标记只影响页面显示，不会进浏览器标签页标题和 SEO 元信息。
**脚注按需显示** —— 全站没有一处用到 `†` 时，那条脚注不会出现；
